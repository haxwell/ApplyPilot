"""Measurement-aware editorial timeline resume template."""
...
TEMPLATE_INFO = {
    "name": "editorial_timeline",
    "display_name": "editorial timeline",
    "description": "A visually distinct resume template for meaning-preservation comparison.",
}
...
class ProfessionalCompactTemplateView:
    """Prepared editorial-timeline template view."""
...
def _render_skills(resume: ResumeRenderModel, mode: SkillsMode, *, selected_skills_max_lines: int = 2) -> str:
    ...
    for section in resume.skills:
        category = str(section.category).strip()
        chips: list[str] = []
        for token in _split_preserving_parentheses(str(section.value)):
            ...
            chips.append(chip)
        if chips:
            grouped_rows.append((category, chips))
    ...
    if mode == "minimal":
        ...
        grouped_rows = [(cat, chips[:6]) for cat, chips in grouped_rows]
    elif mode == "selected":
        max_lines = max(1, selected_skills_max_lines)
        selected_rows: list[tuple[str, list[str]]] = []
        used_lines = 0
        for category, chips in grouped_rows:
            if used_lines >= max_lines:
                break
            per_line = 6
            allowed = max(0, min(len(chips), per_line * (max_lines - used_lines)))
            if allowed <= 0:
                continue
            selected_rows.append((category, chips[:allowed]))
            used_lines += max(1, (allowed + per_line - 1) // per_line)
        grouped_rows = selected_rows
    ...
    rows_parts: list[str] = []
    for category, chips in grouped_rows:
        chips_html = "".join(f'<span class="skill-chip">{chip}</span>' for chip in chips)
        rows_parts.append(
            '<div class="skills-group">'
            f'<div class="skills-group-label">{category}</div>'
            f'<div class="skills-chip-row">{chips_html}</div>'
            "</div>"
        )
    rows = "".join(rows_parts)
    return f'<section class="section"><h2 class="section-title">Technical Skills</h2><div class="skills-grid">{rows}</div></section>'
...
def _render_experience_entry(entry: ResumeEntry, *, bullet_limit: int | None = None) -> str:
    ...
    return (
        ...
        '<div class="entry-rail"><span class="entry-node"></span></div>'
        '<div class="entry-body">'
        f'<h3 class="entry-title">{heading}</h3>{subtitle}{bullet_list}'
        "</div>"
        "</article>"
    )
...
def _render_compact_entry(entry: ResumeEntry, *, one_line: bool = False) -> str:
    ...
    if one_line:
        ...
        return (
            '<article class="compact-entry timeline-entry">'
            '<div class="entry-rail"><span class="entry-node mini"></span></div>'
            f'<div class="compact-meta entry-body">{one_line_text}</div>'
            "</article>"
        )
    ...
    return (
        '<article class="compact-entry timeline-entry">'
        '<div class="entry-rail"><span class="entry-node mini"></span></div>'
        f'<div class="compact-meta entry-body"><strong class="compact-company">{heading}</strong>'
...
def build_html(view: ProfessionalCompactTemplateView) -> str:
    """Build editorial timeline resume HTML from a prepared view."""
    resume = view.model
    ...
    if primary_contact_line:
        contact_lines_html += f'<div class="contact-line">{primary_contact_line}</div>'
    if url_contact_line:
        contact_lines_html += f'<div class="contact-line links-contact">{url_contact_line}</div>'
    sidebar_contact_html = ""
    if contact_lines_html:
        sidebar_contact_html = (
            '<section class="section sidebar-block">'
            '<h2 class="section-title">Contact</h2>'
            f'<div class="contact-block">{contact_lines_html}</div>'
            "</section>"
        )

    title_html = f'<div class="role-title">{resume.title}</div>' if resume.title.strip() else ""
