...
def test_summary_claim_is_rewritten_when_removed_skill_claim_was_unresolved(monkeypatch) -> None:
    model = ResumeRenderModel(
        summary="Experienced designing and implementing highly available services and automation in AWS and Kubernetes environments.",
        skills=[SkillSection(category="Core", value="AWS, Kubernetes, Java")],
        experience=[ResumeEntry(title="Engineer", subtitle="Acme", bullets=["Built Java services."])],
    )
    planning = {
        "allowed_physical_pages": 2,
        "measured_pages_final": 2,
        "claim_coverage": [
            {"claim": "AWS", "coverage_status": "supported", "retained_primary_supporting_evidence_count": 1},
    ...
    def _build_dynamic_planning(**kwargs):
        ...
        for claim in claims:
            key = claim.lower().strip()
            if key in {"aws", "java"}:
                out.append({"claim": claim, "coverage_status": "supported", "retained_primary_supporting_evidence_count": 1})
            elif key == "kubernetes":
                out.append(
                    {
                        "claim": claim,
    ...
    monkeypatch.setattr(pdf_module, "_build_planning_with_evidence", _build_dynamic_planning)
    updated_model, _html, _prepared, updated = _apply_evidence_aware_skill_replacements(
        model=model,
        html="<html/>",
        prepared=SimpleNamespace(),
        planning=planning,
        template_name="professional_compact",
        job_description="AWS backend role.",
        skills_selection={"retained_skills": [{"skill": "AWS", "score": 95.0}, {"skill": "Kubernetes", "score": 94.0}, {"skill": "Java", "score": 93.0}], "min_count": 2},
    )

    assert "Kubernetes" not in extract_all_skill_claims(updated_model)
    assert "Kubernetes" not in updated_model.summary
    assert "AWS" in updated_model.summary
    assert updated.get("summary_claim_repairs")
    assert any(item.get("claim") == "Kubernetes" for item in updated["summary_claim_repairs"])
    assert "Kubernetes" in updated.get("removed_or_rewritten_summary_claims", [])
    assert updated.get("summary_claims_final_unresolved", []) == []


def test_supported_skill_claim_can_remain_in_summary_after_summary_repair(monkeypatch) -> None:
    model = ResumeRenderModel(
        summary="Built Java and AWS services with high reliability.",
        skills=[SkillSection(category="Core", value="AWS, Kubernetes, Java")],
        experience=[ResumeEntry(title="Engineer", subtitle="Acme", bullets=["Built Java services."])],
    )
    planning = {
    ...
    assert "Java" in updated_model.summary
    assert "AWS" in updated_model.summary
    assert "Kubernetes" not in updated_model.summary
    assert updated.get("summary_claims_final_unresolved", []) == []
...
def test_summary_rewrite_restores_oxford_conjunction_after_claim_removal(monkeypatch) -> None:
    model = ResumeRenderModel(
        summary="Experienced building resilient systems using Java, Spring Boot, Kubernetes, and AWS.",
        skills=[SkillSection(category="Core", value="Java, Spring Boot, Kubernetes, AWS")],
        experience=[ResumeEntry(title="Engineer", subtitle="Acme", bullets=["Built Java Spring Boot services on AWS."])],
    )
    planning = {
        "allowed_physical_pages": 2,
        "measured_pages_final": 2,
        "claim_coverage": [
            ...
            {"claim": "Kubernetes", "coverage_status": "weak_summary_only", "retained_primary_supporting_evidence_count": 0},
            {"claim": "AWS", "coverage_status": "supported", "retained_primary_supporting_evidence_count": 1},
        ],
        "unsupported_visible_claims": [],
        "weak_visible_claims": ["Kubernetes"],
    ...
    updated_model, _html, _prepared, _updated = _apply_evidence_aware_skill_replacements(
        ...
        planning=planning,
        template_name="professional_compact",
        job_description="Backend platform role.",
        skills_selection={"retained_skills": [{"skill": "Java", "score": 95.0}], "min_count": 1},
    )
    assert "using Java, Spring Boot, and AWS." in updated_model.summary
    assert "Kubernetes" not in updated_model.summary
...
def test_compound_skill_removes_unsupported_subclaims(monkeypatch) -> None:
    ...
    assert updated.get("rendered_visible_skill_names")
    assert updated.get("rendered_visible_skill_count") == len(updated.get("rendered_visible_skill_names", []))
...
def test_inferred_grouping_from_flat_skills_builds_parenthetical_form(monkeypatch) -> None:
    model = ResumeRenderModel(
        skills=[SkillSection(category="Core", value="AWS EC2, AWS Lambda, Route53, Java")],
        experience=[ResumeEntry(title="Engineer", subtitle="Acme", bullets=["Built deployment workflows using AWS EC2, Lambda, Route53, and DNS automation."])],
    )
    planning = {
        "allowed_physical_pages": 2,
        "measured_pages_final": 2,
    ...
    def _coverage_for_claims(*, claims, **_kwargs):
        ...
        for claim in claims:
            key = str(claim).lower().strip()
            supported_keys = {"aws ec2", "aws lambda", "aws route53", "aws", "route53", "java"}
            supported = key in supported_keys
            retained = 1 if supported else 0
            status = "supported" if supported else "unsupported"
            if key in {"aws ec2", "aws lambda", "aws route53", "aws", "route53"}:
                top = ["Acme: Built deployment workflows using AWS EC2, Lambda, Route53, and DNS automation."]
            elif key == "java":
    ...
    new_model, _html, _prepared, updated = _apply_evidence_aware_skill_replacements(
        ...
        template_name="professional_compact",
        job_description="AWS platform role.",
        skills_selection={"retained_skills": [{"skill": "AWS EC2", "score": 95.0}, {"skill": "AWS Lambda", "score": 94.0}, {"skill": "Route53", "score": 93.0}, {"skill": "Java", "score": 90.0}], "min_count": 1},
    )
    claims = extract_all_skill_claims(new_model)
    assert "AWS (EC2, Lambda, Route53)" in claims
    assert "AWS EC2" not in claims
    assert "AWS Lambda" not in claims
    assert "Route53" not in claims
    assert updated.get("unsupported_visible_claims_final", []) == []
    assert updated.get("weak_visible_claims_final", []) == []
    assert updated.get("grouped_skill_repairs")


def test_inferred_grouping_non_programming_keeps_only_supported_subclaims(monkeypatch) -> None:
    model = ResumeRenderModel(
        skills=[SkillSection(category="Core", value="Adobe Photoshop, Adobe Illustrator, Adobe InDesign, Customer Service")],
        experience=[ResumeEntry(title="Designer", subtitle="Acme", bullets=["Created assets in Adobe Photoshop and Illustrator for campaigns."])],
    )
    planning = {
        "allowed_physical_pages": 2,
        "measured_pages_final": 2,
    ...
    def _coverage_for_claims(*, claims, **_kwargs):
        ...
        for claim in claims:
            key = str(claim).lower().strip()
            supported = key in {"adobe photoshop", "adobe illustrator", "adobe", "customer service"}
            retained = 1 if supported else 0
            out.append(
                ClaimCoverage(
                    claim=str(claim),
    ...
    new_model, _html, _prepared, updated = _apply_evidence_aware_skill_replacements(
        ...
        planning=planning,
        template_name="professional_compact",
        job_description="Creative design role.",
        skills_selection={"retained_skills": [{"skill": "Adobe Photoshop", "score": 95.0}, {"skill": "Adobe Illustrator", "score": 94.0}, {"skill": "Adobe InDesign", "score": 93.0}], "min_count": 1},
    )
    claims = extract_all_skill_claims(new_model)
    assert "Adobe (Photoshop, Illustrator)" in claims
    assert all("InDesign" not in claim for claim in claims)
    assert "InDesign" in updated.get("unsupported_compound_subclaims_removed", [])


def test_grouped_skill_does_not_attach_unrelated_supported_standalones(monkeypatch) -> None:
    model = ResumeRenderModel(
        skills=[SkillSection(category="Core", value="AWS EC2, AWS Lambda, Microservices, MySQL, Java")],
        experience=[
            ResumeEntry(
                title="Engineer",
                subtitle="Acme",
                bullets=[
                    "Implemented deployment automation with AWS EC2, Lambda, Route53, and DNS scripting.",
                    "Built microservices and MySQL-backed APIs for internal platforms.",
                ],
            )
        ],
    )
    ...
    def _coverage_for_claims(*, claims, **_kwargs):
        ...
        for claim in claims:
            key = str(claim).lower().strip()
            supported = key in {"aws", "aws ec2", "aws lambda", "ec2", "lambda", "microservices", "mysql", "java"}
            retained = 1 if supported else 0
            if key in {"aws", "aws ec2", "aws lambda", "ec2", "lambda"}:
                top = ["Acme: Implemented deployment automation with AWS EC2, Lambda, Route53, and DNS scripting."]
            elif key in {"microservices", "mysql"}:
                top = ["Acme: Built microservices and MySQL-backed APIs for internal platforms."]
            elif key == "java":
                top = ["Acme: Built Java services."]
            else:
                top = []
    ...
    new_model, _html, _prepared, _updated = _apply_evidence_aware_skill_replacements(
        model=model,
        html="<html/>",
        prepared=SimpleNamespace(),
        planning=planning,
        template_name="professional_compact",
        job_description="Cloud platform role.",
        skills_selection={"retained_skills": [{"skill": "AWS EC2", "score": 95.0}, {"skill": "AWS Lambda", "score": 94.0}, {"skill": "Microservices", "score": 93.0}, {"skill": "MySQL", "score": 92.0}], "min_count": 1},
    )
    claims = extract_all_skill_claims(new_model)
    assert "AWS (EC2, Lambda)" in claims
    assert all("Microservices" not in claim for claim in claims if claim.startswith("AWS"))
    assert all("MySQL" not in claim for claim in claims if claim.startswith("AWS"))
    assert "Microservices" in claims
    assert "MySQL" in claims


def test_inferred_grouping_recovers_dropped_supported_subclaims_from_pool(monkeypatch) -> None:
    model = ResumeRenderModel(
        skills=[SkillSection(category="Core", value="AWS EC2, AWS S3, Java")],
        experience=[
            ResumeEntry(
                title="Engineer",
                subtitle="Acme",
                bullets=["Implemented deployment automation with AWS EC2, Lambda, Route53, and DNS scripting."],
            )
        ],
    )
    planning = {
    ...
    new_model, _html, _prepared, _updated = _apply_evidence_aware_skill_replacements(
        ...
        skills_selection={
            ...
            "dropped_skills": ["AWS Lambda", "Route53"],
            "min_count": 1,
        },
    )
    claims = extract_all_skill_claims(new_model)
...
def test_grouped_skill_consolidates_duplicate_parent_groups(monkeypatch) -> None:
    model = ResumeRenderModel(
        skills=[SkillSection(category="Core", value="AWS (EC2, Lambda), AWS (EC2, Lambda, CI/CD), Java")],
        experience=[ResumeEntry(title="Engineer", subtitle="Acme", bullets=["Built deployment workflows using AWS EC2, Lambda, and DNS automation."])],
    )
    planning = {
        "allowed_physical_pages": 2,
        "measured_pages_final": 2,
    ...
    def _coverage_for_claims(*, claims, **_kwargs):
        ...
        for claim in claims:
            key = str(claim).lower().strip()
            supported = key in {"aws", "ec2", "lambda", "aws ec2", "aws lambda", "java"}
            retained = 1 if supported else 0
            out.append(
                ClaimCoverage(
                    claim=str(claim),
                    ...
                    secondary_supporting_evidence_count=0,
                    coverage_status="supported" if supported else "unsupported",
                    top_supporting_evidence=["Acme: Built deployment workflows using AWS EC2, Lambda, and DNS automation."] if supported else [],
                )
            )
        return out
    ...
    new_model, _html, _prepared, _updated = _apply_evidence_aware_skill_replacements(
        ...
        template_name="professional_compact",
        job_description="Cloud platform role.",
        skills_selection={"retained_skills": [{"skill": "AWS (EC2, Lambda)", "score": 95.0}, {"skill": "AWS (EC2, Lambda, CI/CD)", "score": 94.0}], "min_count": 1},
    )
    claims = extract_all_skill_claims(new_model)
...
def test_taxonomy_evidence_discovery_adds_supported_missing_child_without_reintroducing_removed_one(monkeypatch) -> None:
    model = ResumeRenderModel(
        skills=[SkillSection(category="Core", value="AWS EC2, AWS S3, AWS Lambda, Java")],
        experience=[
            ResumeEntry(
                title="Engineer",
                subtitle="Acme",
